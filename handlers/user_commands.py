...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Database):
    ...
    if db.user_exists(user_id):
        await update.message.reply_text(
            ...
            "You have already initialized your account.\n"
            "Send /help to view available commands."
        )
        return

    # Handle invite info if present
...
async def checkin_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Database):
    """Handle the /qd daily check-in command."""
    user_id = update.effective_user.id
    ...
    if not db.can_checkin(user_id):
        await update.message.reply_text("❌ You've already checked in today. Please come back tomorrow.")
        return

    # Second layer check (atomic in DB)
    if db.checkin(user_id):
        ...
        # If DB layer returns False, the user already checked in today
        await update.message.reply_text("❌ You've already checked in today. Please come back tomorrow.")
...
async def invite_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Database):
    ...
    await update.message.reply_text(
        f"🎁 Your invite link:\n{invite_link}\n\n"
        "Earn 2 points for each successful registration through your link."
    )


async def use_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Database):
    """Handle the /use command for redeeming card keys."""
    if await reject_group_command(update):
        return
    ...
    if result is None:
        await update.message.reply_text("The card key does not exist. Please check and try again.")
    elif result == -1:
        ...
        user = db.get_user(user_id)
        await update.message.reply_text(
            f"Card key redeemed successfully!\nPoints gained: {result}\nCurrent points: {user['balance']}"
        )
