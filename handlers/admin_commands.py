...
async def addbalance_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Database):
    """Handle the /addbalance command for adding points."""
    ...
    try:
        ...
        if not db.user_exists(target_user_id):
            await update.message.reply_text("The user does not exist.")
            return
...
async def block_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Database):
    """Handle the /block command to block a user."""
    if await reject_group_command(update):
        return
...
async def white_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Database):
    """Handle the /white command to unblock a user."""
    if await reject_group_command(update):
        return
...
async def blacklist_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Database):
    """Handle the /blacklist command to view blocked users."""
    if await reject_group_command(update):
        return
...
async def genkey_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Database):
    """Handle the /genkey command to create card keys."""
    if await reject_group_command(update):
        return
    ...
    if not context.args or len(context.args) < 2:
        await update.message.reply_text(
            "Usage: /genkey <code> <points> [uses] [expire_days]\n\n"
            "Examples:\n"
            "/genkey wandouyu 20 - Generates a 20-point key (single use, no expiration)\n"
            "/genkey vip100 50 10 - Generates a 50-point key (10 uses, no expiration)\n"
            "/genkey temp 30 1 7 - Generates a 30-point key (single use, expires in 7 days)"
        )
        return
    ...
    try:
        ...
        if db.create_card_key(key_code, balance, user_id, max_uses, expire_days):
            ...
            msg += f"\nHow to redeem: /use {key_code}"
            await update.message.reply_text(msg)
        else:
            await update.message.reply_text("The card key exists or creation failed. Please use a different code.")
    except ValueError:
        await update.message.reply_text("Invalid parameters. Please enter valid numbers.")


async def listkeys_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Database):
    """Handle the /listkeys command to view card keys."""
    if await reject_group_command(update):
        return
    ...
    if len(keys) > 20:
        msg += f"\n(Showing only the first 20 of {len(keys)} keys)"

    await update.message.reply_text(msg)


async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Database):
    """Handle the /broadcast command to send notifications to all users."""
    if await reject_group_command(update):
        return
    ...
    for uid in user_ids:
        try:
            ...
            success += 1
            await asyncio.sleep(0.05)  # Rate limit to avoid throttling
