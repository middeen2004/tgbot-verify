"""MySQL database implementation using a provided MySQL server for storage."""
import logging
...
class MySQLDatabase:
    ...
    def __init__(self):
        ...
        # Read configuration from environment variables (preferred) or fall back to defaults
        self.config = {
    ...
    def get_user(self, user_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            if row:
                # Create a new dict and convert datetime values to ISO formatted strings
                result = dict(row)
    ...
    def checkin(self, user_id: int) -> bool:
        """User check-in (fixes the previous unlimited check-in bug)."""
        conn = self.get_connection()
        cursor = conn.cursor()

        try:
            # Use an atomic SQL operation to avoid race conditions
            # Only update when last_checkin is NULL or the date is before today
            cursor.execute(
                """
                UPDATE users
            ...
            # Confirm the update succeeded (affected_rows > 0 means success)
    ...
    def use_card_key(self, key_code: str, user_id: int) -> Optional[int]:
        """Use a card key and return the awarded balance."""
        conn = self.get_connection()
        cursor = conn.cursor(DictCursor)
        ...
        try:
            ...
            # Check whether the user has already used this key
            cursor.execute(
    ...
    def get_card_key_info(self, key_code: str) -> Optional[Dict]:
        """Get details for a specific card key."""
        conn = self.get_connection()
        cursor = conn.cursor(DictCursor)
...
# Global instance alias for compatibility with the SQLite version
