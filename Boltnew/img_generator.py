"""PNG teacher credential generator module - Bolt.now / PSU."""
import random
from datetime import datetime


def generate_psu_id():
    """Generate a random 9-digit PSU ID."""
...
def _get_browser_context():
    """Get or create the browser context (singleton)."""
...
def _html_to_png(html_content: str, width: int = 1200, height: int = None) -> bytes:
    """Convert HTML to a PNG screenshot (reusing the browser instance)."""
    try:
        context = _get_browser_context()
        page = context.new_page()

        try:
            # Set HTML content directly, using domcontentloaded for speed
            page.set_content(html_content, wait_until='domcontentloaded')

            # Wait for images to load (if external images exist)
...
def generate_employment_letter_html(
    ...
    """Generate employment verification letter HTML."""
    name = f"{first_name} {last_name}"
    now = datetime.now()
...
def _html_to_png_batch(html_list: list[tuple[str, int, int]]) -> list[bytes]:
    """
    Generate multiple PNGs concurrently (optimized).

    Args:
        html_list: [(html_content, width, height), ...]

    Returns:
        list[bytes]: List of PNG byte arrays
    """
    import asyncio
    from playwright.async_api import async_playwright

    async def render_single(html_content: str, width: int, height: int):
        """Render a single image asynchronously."""
...
def generate_images(first_name: str, last_name: str, school_id: str = '2565'):
    """
    Generate two PNGs: faculty card and employment letter (concurrent).

    Args:
        first_name: First name
        last_name: Last name
        school_id: School ID (kept for interface consistency)
